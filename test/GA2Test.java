...
public class GA2Test {
  ...
  public static void main(String[] args) {
    ...
    println("start");
    GA2.Param param = new GA2.Param() {
      @Override
      public GA2.IEval I_EVAL() {
        return (gene) -> {
          float acc = 0f;
...
//              acc += Math.abs(target[i] - gene[i]) + 10;
      ...
      @Override
      public GA2.IRandomGene I_RANDOM_GENE() {
        return new GA2.IRandomGene() {
          @Override
      ...
      @Override
      public GA2.IMutation I_MUTATION() {
        return new GA2.IMutation() {
          @Override
          public void mutation(GA2.GARuntime gaRuntime, byte[] gene, ThreadLocalRandom r) {
    ...
    final int n_pop = 100;
    GA2 ga = new GA2(new GA2.GARuntime(n_pop, target.length, 0.25f, 0.9f, 1f / target.length * 4), param);
    ga.init();
    final float[] best = {1f};
    int turn = 0;
    for (; best[0] != 0; turn++) {
